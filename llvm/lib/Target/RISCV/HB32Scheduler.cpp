...
/// Flag to enable the custom scheduler for the HB32 Vanilla core.
static cl::opt<bool>
    HB32Sched("hb32sched",
              cl::desc("Enable HB32 Vanilla Core's custom scheduler"),
              cl::init(true), cl::Hidden);

/// Create the custom scheduler for HB32 functions with FP load streams.
ScheduleDAGInstrs *llvm::createHB32Scheduler(MachineSchedContext *C) {
  if (!HB32Sched)
    return nullptr;

  // The load-first policy is intended to expose latency for FP data streams.
  // Applying it to integer-only code can bunch local loads immediately before
  // their users and make the generic scheduler's ordering worse.
  for (const MachineBasicBlock &MBB : *C->MF)
    for (const MachineInstr &MI : MBB)
      if (MI.getOpcode() == RISCV::FLW)
  ...
  do {
    ...
    if (!SU) {
      for (SUnit *SUi : Top.Available) {
        ...
        if (MI->mayLoad() && !memops.empty() &&
            memops[0]->getAddrSpace() == 1) {
          LLVM_DEBUG(dbgs() << "HB32Scheduler bumping load (" << SUi->NodeNum
                            << ") " << *MI);
          SU = SUi;
          break;
        }
